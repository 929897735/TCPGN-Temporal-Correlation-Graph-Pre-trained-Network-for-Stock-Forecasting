...
from utils.train_utils import train_predict
from model.mlp_model import mlp_model
from model.model import TCGPN

settings = toml.load("./config/predict_config.toml")
...
if __name__ == "__main__":
    ...
    pretrain_model = torch.load(
        os.path.join(
            settings["pretrain_model_path"], settings["pretrain_model_file_name"]
        )
    )

    if settings['model_name']=='mlp':
        model = mlp_model(
            settings=settings["mlp_model_config"], pretrain_model=pretrain_model
        )
